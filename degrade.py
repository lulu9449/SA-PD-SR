...
sampler = Bicubic(22, normal_bicubic=True).cuda()
sampler_ori = bicubic()

path = "/data/users/luluzhang/datasets/DIV2K/DIV2K_valid_HR"
imgs_name = os.listdir(path)
imgs_name.sort()
imgs_name = [os.path.join(path, name) for name in imgs_name]
for img_name in imgs_name:
    ...
    img_lr, _ = sampler(img_hr, [lowH, lowW])
